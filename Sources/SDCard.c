...
int Test_SDCard_Write_Mult_Blocks(uint32_t sector, uint8_t buffer[][SDCARD_SECTOR_SIZE], int block_cnt)
{
    ...
    Set_DSPI_PUSHR(&DSPI_1_Device_Data, SDCARD_DSPI_PUSHR_CONT, SDCARD_DSPI_PUSHR_PCS);
    DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send, NULL, 3);
    for (j = 0; j < block_cnt; j++)
    {
        DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send + 8, NULL, 1);
        for (i = 0; i < SDCARD_SECTOR_SIZE / DSPI_ASYNC_SEND_DATA_MAX_LENGTH; i++)
        {
            DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, &buffer[j][DSPI_ASYNC_SEND_DATA_MAX_LENGTH * i], NULL, DSPI_ASYNC_SEND_DATA_MAX_LENGTH);
        }
        DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send, NULL, 2);
        DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send, &res, 1);
        if ((res & 0b00011111) != 0b00000101)
    ...
    DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send + 9, NULL, 1);
    while(1)
    {
        uint8_t res = 0x00;
        DSPI_SYNC_Send_and_Receive_Data(&DSPI_1_Device_Data, send, &res, 1);
